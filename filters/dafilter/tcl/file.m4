...
match_file_noappend($rc(awips) s s1, swody([0-9]), nwx/spc/day$s1,
latest.day$s1)
...
# Watch County List
matchstop_file($rc(awips), ^sev[0-9], $gdatadir/nwx/spc/sev, $ymdh.sev)
...
nwx/tropical/trpcycu, $ymdh.trpcycu)
...
match_file_noappend($rc(wmoid), asus01, nwx/hpc/fronts, latest.front)
matchstop_file($rc(wmoid), asus01, nwx/hpc/fronts, $ymdh.front)

# Coded Fronts Forecast
match_file_noappend($rc(wmoid), fsus02, nwx/hpc/forecast, latest.fcst)
