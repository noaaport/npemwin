...
matchstop_file($rc(wmoid),
(^s(hv|hxx|s[^x]))|(^sx(vd|v.50|us(2[0-3]|08|40|82|86)))|(^y[ho]xx84),
ship, $ymdh.sb)

# 6 Hour Ship Observations
match_file($rc(wmoid), ^s[imn]v[^gins]|^s[imn]w[^kz], ship, $ymdh.sb)
matchstop_file($rc(wmoid), ^s[imn]v[^gins]|^s[imn]w[^kz], ship6hr, $ymdh.ship)

# Wave Observations (from the ncep pqact.conf)
matchstop_file($rc(wmoid), ^sxvx[46], wavobs, $ymdh.wvob)

# upper air reports and dropsonde reports - dcuair
matchstop_file($rc(wmoid), ^u[abcdefghijklmnpqrstwxy], upperair, $ymdh.upa)
matchstop_file($rc(wmoid), ^uz, drops, $ymdh.drop)

# ngm Mos - dcnmos
matchstop_file($rc(wmoid), ^fo(us14|ak1[34]|ak2[5-9]), mos, $ymdh.nmos)

# Decoder for GFS MOS - dcgmos
matchstop_file($rc(wmoid), fous2[1-6]|foak3[7-9]|fopa20, mos, $ymdh.gmos)

# Decoder for GFSX MOS
matchstop_file($rc(wmoid), feus2[1-6]|feak3[7-9]|fepa20, mos, $ymdh.xmos)

# Decoder for global sea-ice drift bulletins - dcidft
matchstop_and_file($rc(wmoid), fzxx41, $rc(awips1), idm, idft, $ymdh.idft)

# Alaska sea ice bulletins - dcidft
matchstop_and_file($rc(wmoid), fzak41, $rc(awips1), idm, idft, $ymdh.idak)
...
match_file($rc(fname), (kmkc_wwus40|kwns_wwus30),
storm/watches, $ym.watches)

# dcwarn
match_file($rc(awips1), tor|svr|ffw, storm/warn, $ymdh.warn)

# dcwtch
match_file($rc(wmoid), ^wwus(40|08)|wous20|wwus30,
storm/wtch, $ymdh.wtch)

# dcwcp
match_and_file($rc(wmoid), wwus60, $rc(station), kwns,
storm/wcp, $ymdh.wcp)
...
matchstop_file($rc(awips1), scd, scd, $ymd.scd)

# Aircraft Observations - dcacft
matchstop_file($rc(fname), kawn_(xrxx84|yixx84)|_u[abdr], acft, $ymdh.acf)

# Airmets - dcairm
matchstop_file($rc(wmoid), ^waus01, airm, $ymdh.airm)

# International Sigmets - dcisig
matchstop_and_file($rc(station), kkci|phfo|pawu|pgum|panc,
$rc(wmoid), ^w[scv](pn|nt|pa|ak),
isig, $ymdh.isig)

# Non-convective Sigmets - dcncon
matchstop_and_file($rc(station), kkci, $rc(wmoid), wsus01|w[scv]us0[1-6],
ncon, $ymdh.sgmt)

# Convective Sigmets - dccsig
matchstop_and_file($rc(station), kkci,
$rc(wmoid), wsus4[0-2]|wcpa3[1-5]|wsus3[1-3],
csig, $ymdh.conv)

# TAFs - dctaf
matchstop_file($rc(wmoid), ^ft, taf, $ymdh.taf)

# Regional Digital Forecasts/Point Forecast Matricies - dcrdf
matchstop_file($rc(wmoid), ^fous5[1-5], rdf, $ymdh.rdf)
