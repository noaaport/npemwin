...
int main(int argc, char **argv){
  ...
  char *optstr = "e:";
  char *usage = "npemwinudpsend [-e str] <host> <port>";
  int c;
  int status = 0;
...
static int udpsend(void){
  ...
  if(g.opt_str != NULL){
    if(sendto(fd, g.opt_str, strlen(g.opt_str), 0,
	      (struct sockaddr*)readeraddr, readeraddr_len) == -1)
      status = -1;
  }else{
    while((fgets(str, STRSIZE, stdin) != NULL) && (status == 0)){
      if(sendto(fd, str, strlen(str), 0, (struct sockaddr*)readeraddr,
		readeraddr_len) == -1)
	status = -1;
    }
  }
  if(status == -1)
    err(1, "Error in sendto()");
