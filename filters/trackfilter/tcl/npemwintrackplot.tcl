...
# $Id$
#
# usage: npemwintrackplot.tcl [-D] [-i <datafile>] [-o <outputfile>]
...
set usage {usage: npemwintrackplot.tcl [-D] [-i <datafile>] [-o <outputfile>]
    [-r <region>] [-s <trackscript>] [-m <mapscript>] [-y <year>] <name>};
...
# behace well when it encounters an error; instead of writing the errmsg
# and quiting, it writes the error and then a prompt (even with the [-b]
# switch). The result is "exec hangs. To work around this we execute it
# via a pipe.
